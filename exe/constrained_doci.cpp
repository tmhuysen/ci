...
int main() {
    std::ofstream outfile ("co.cdoci");
    outfile << "multiplier" << " \t RHF energy   " << " \t C population RHF    "<< " \t C DOCI energy  " << " C population DOCI " <<std::endl;
    ...
    for(double i = -1;i<1.0;i += 0.1){
        ...
        double energy = doci.solveConstrained(numopt::eigenproblem::SolverType::ARMADENSE,ao_set,0);
    ...
    outfile <<std::endl<<" repulsion " << CO.calculateInternuclearRepulsionEnergy()<< " doci multiplier : "<<0;
    outfile.close();
}
