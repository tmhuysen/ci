...
int main() {
    std::ofstream outfile ("NO_FCI_Constrained_ergo_dojo.data");
    ...
    for(double i : files){
        ...
        for(double x = -d;x<d+0.1;x += 0.1){
            try{
                ...
                std::string filename2 = "../../exe/xyz/no_" + s +"_PB";
                libwint::SOMullikenBasis so_basis_temp(filename,10);


                libwint::Molecule no (filename2+".xyz",1);
                libwint::AOBasis ao_basis(no,"STO-3G");
                ao_basis.calculateIntegrals();
                libwint::SOMullikenBasis so_basis(ao_basis,so_basis_temp.get_C().transpose());
